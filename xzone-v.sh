...
#		#############################################################
#		#															#
#		#	XZone Bash Script 0.3 - Verbose - Scytheri0n			#
#		#															#
...
function testForInteger {				#	Makes sure $inputTime is an integer
	...
	if ! [[ $inputTime =~ $re ]] ; then
 	  ...
 	  printf "Would you like to start at a specific MAC? (This is useful\nif you have terminated the script and wish to pick up where you left off)\n"
 	  read inputTime
 	  testForInteger
	fi
}
	...
	case $startPoint in
      ...
      ( * )                
        ...
        2)
        	endHex=$startPoint
        	endHex=$((0x${endHex}))
        	echo "Starting at "$MAC$startPoint
        	;;
        *)
...
echo "Dropping back to standard MACs."
